...
const gameOverRegex = /(ende|end|over)/;
...
const testVariableInitialization = async function (t) {
    ...
    t.end();
    console.log("stop step3_1");
};

const testBowlInitialization = async function (t) {
    console.log("start step3_2");
    t.seedScratch('seed');
...
const testMoveBowl = async function (t, testDetails = false) {
    ...
    t.end();
    console.log("stop step3_4");
};

const testMoveBowlDetails = async function (t) {
    console.log("start step3_5");
    t.seedScratch('seed');

    await testMoveBowl(t, true);
    t.end();
    console.log("stop step3_5");
};
...
const testTimerTick = async function (t) {
    ...
    t.end();
    console.log("stop step3_26");
};

const testTimerGameOver = async function (t) {
    console.log("start step3_27");
    t.seedScratch('seed');

    let {time, apple, bowl} = getSpritesAndVariables(t, ['time', 'apple', 'bowl']);
    ...
    for (let index = 0; index < 27; index++) {
        t.assert.ok(time.value > 0, "Time was 0 before 30 seconds were over.");
        await t.runForTime(1000);
    }

    /* Run game for 6 more seconds to game over. */
    await t.runUntil(() => time.value == 0 || time.value == undefined, 6000);

    // after 'stop all' block all variables are undefined.
    t.assert.ok(time.value == undefined || time.value == 0, "Time must be 0 after 30 seconds");
    ...
    t.end();
    console.log("stop step3_27");
};

const testTimerGameOverMessage = async function (t) {
    t.seedScratch('seed');
    console.log("start step3_28");
